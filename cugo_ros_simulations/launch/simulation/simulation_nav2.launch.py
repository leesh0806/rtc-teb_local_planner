...
def generate_launch_description():
    ...
    source_map_path = os.path.abspath(os.path.join(source_dir, 'worlds', 'map', 'my_map.yaml'))
    installed_map_path = os.path.abspath(os.path.join(package_dir, 'worlds', 'map', 'my_map.yaml'))
    ...
    # 최종 경로를 절대 경로로 확실히 변환
    default_map_path = os.path.abspath(default_map_path)
    
    # 기본 파라미터 파일 경로 - 절대 경로 사용
    ...
    nav2_launch_file_dir = os.path.abspath(os.path.join(get_package_share_directory('nav2_bringup'), 'launch'))
    rviz_config_dir = os.path.abspath(os.path.join(package_dir, 'rviz', 'nav2_default_view.rviz'))
